...
export const Layout = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <div className="flex h-screen bg-background overflow-hidden outline-none">
      <aside className="w-[250px] bg-navy flex flex-col shrink-0">
        ...
        <nav className="flex-1 px-4 space-y-1 overflow-y-auto">
          {navItems.map((item) => (
            <NavLink
              ...
              className={({ isActive }) =>
                `
                flex items-center gap-3 px-4 py-3 rounded-xl font-medium text-[14px] transition-all duration-200
                ${
                  isActive
        ...
        <div className="p-6 pt-4 border-t border-white/5">
          <div className="bg-white/5 p-4 rounded-2xl mb-4">
            ...
            <p className="text-gold text-[10px] font-bold uppercase">
              {user?.role === 'owner' ? 'System Owner' : 'Shopkeeper'}
            </p>
          </div>
        ...
        <header className="h-[90px] bg-white border-b border-border-subtle flex items-center justify-between px-8 shrink-0">
          <div>
            ...
            <p className="text-[13px] text-slate-500 font-medium">
              Location: {business?.address || 'Main Branch'}
            </p>
          </div>

          <div className="flex items-center gap-8">
            <div className="text-right hidden sm:block">
              <p className="text-[11px] text-slate-400 uppercase tracking-widest font-black mb-0.5">
                Live Revenue
              </p>
              <p className="text-xl font-bold text-navy">$0.00</p>
            </div>

            <div className="h-10 w-[1px] bg-border-subtle hidden sm:block" />
